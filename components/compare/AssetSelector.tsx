...
export const AssetSelector: React.FC<AssetSelectorProps> = ({ selectedIds, onChange, registry }) => {
  ...
  return (
    <div className="w-full flex flex-col gap-3 relative" ref={dropdownRef}>
      {/* Selected Pills + Add Button Container */}
      <div className="flex flex-wrap items-center gap-2">
         ...
         <div className="relative inline-block">
           ...
           <AnimatePresence>
             {showDropdown && (
               <>
                 {/* Backdrop overlay for mobile to handle closing and focus */}
                 <motion.div 
                   initial={{ opacity: 0 }}
                   animate={{ opacity: 1 }}
                   exit={{ opacity: 0 }}
                 ...
                 {/* Dropdown Panel - Anchored to button on desktop, Centered on mobile */}
                 <motion.div 
                   initial={{ opacity: 0, scale: 0.95, y: -10 }}
                   animate={{ opacity: 1, scale: 1, y: 0 }}
                   exit={{ opacity: 0, scale: 0.95, y: -10 }}
                   transition={{ duration: 0.2, ease: "easeOut" }}
                   className="fixed sm:absolute top-[10vh] sm:top-full left-4 right-4 sm:left-1/2 sm:right-auto sm:-translate-x-1/2 mt-2 sm:w-[500px] max-w-[95vw] sm:max-w-[500px] rounded-2xl z-[100] flex flex-col overflow-hidden bg-surface border border-border shadow-[0_20px_60px_rgba(0,0,0,0.6)] origin-top transform-gpu"
                 >
                  {/* Search */}
                  ...
                  {/* Asset List */}
                  <div className="max-h-[320px] sm:max-h-[400px] overflow-y-auto py-2 px-2 flex flex-col gap-0.5 bg-surface"
